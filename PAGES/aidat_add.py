...
class AidatAdd(QWidget):
    ...
    def combo_ekle(self):     
        ...
        cur = db.cursor()
        cur.execute("SELECT * FROM ogrenciler WHERE tc=?",(self.tcid,))
        rows=cur.fetchall()
    ...
    def add_aidat(self):
        try:
            ...
            self.c.execute("INSERT INTO aidatlar (tc,isim,soyad,ay,yıl,ucret) VALUES (?,?,?,?,?,?)",(self.tcid,isim,soyad,ay,yil,ucret))
            self.conn.commit()
            self.conn.close()
            self.eklendi_info()
            self.close()
