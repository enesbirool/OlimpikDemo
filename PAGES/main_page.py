...
class MainPage(QMainWindow):
    ...
    def open_aidat_page(self):
        if len(self.ui.ogr_tc_edit.text())>=1:
            self.aidat_add_page.show()
            self.aidat_add_page.tcid=self.ui.ogr_tc_edit.text()
            self.aidat_add_page.combo_ekle()
        else:
            self.ui.statusBar.showMessage("Aidat İçin Öğrenci Seçmelisiniz", 9000)
    ...
    def generate_card(self):
        ...
        id.text((360, 620),self.id , fill="black", font=font)
    ...
    def student_add(self):
        ...
        if (len(tc) >10 and len(isim) != 0 and len(numara) != 0 and len(bitis) != 0):
            try:
                ...
                self.conn = sql.connect("./db/mxsoftware.db")
                self.c = self.conn.cursor()
                cevap=self.c.execute("SELECT COUNT(*) FROM ogrenciler")
                for i in cevap:
                    count=i[0]+1
                if count<=15:
                    ...
                    QMessageBox.question(self, 'İnfo Page', "Deneme Sürümünüz 15 kişi İle Sınırlıdır Lütfen geliştirici ile iletişime geçiniz...",
                                         QMessageBox.Ok)
